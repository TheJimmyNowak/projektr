...
ggplot(clustered, aes(x = clustered$`Arrival Delay in Minutes`, y = clustered$`Departure Delay in Minutes`)) +
  ...
  labs(
    x = "Czas opóźnienia przylotu",
    y = "Czas opóźnienia odlotu"
  )


```
```{r}
df[,c("Departure", "Arrival")] = dat[, c("Departure Delay in Minutes", "Arrival Delay in Minutes")]
set.seed(1)
...
model <- lm(Arrival ~ Departure, data = train)
summary(model)
```
# It'S time to make some predictions
```{r}
predictions <- predict(model, newdata = test[,c("Departure")])

#create scatterplot with fitted regression line
ggplot(test, aes(x = Arrival, y = Departure)) +
  geom_point() +
  stat_smooth(method = "loess", se=TRUE)
```

```{r}
mean_of_snacks <- mean(clustered$`Food and drink`)
mean_of_snacks
mean_of_seat_comfort <- mean(clustered$`Seat comfort`)
mean_of_seat_comfort
mean_of_seat_LRS <- mean(clustered$`Leg room service`)
mean_of_seat_LRS
```